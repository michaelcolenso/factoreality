"""Local agentic harness responses for offline pipeline execution.

This module intentionally does not call any outside LLM API.  It turns the
pipeline prompts into deterministic, file-backed stage output so the surrounding
agent harness can orchestrate and execute the workflow locally.
"""
...
"""
    ...
    def _outline(user_message: str) -> str:
        min_words, max_words = AgenticHarness._extract_word_range(user_message)
...
# Backwards-compatible name for older tests/imports.
FakeLLM = AgenticHarness
